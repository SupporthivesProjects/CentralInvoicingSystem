...
class LaravelController extends Controller
{
    ...
    public function randomProducts(Request $request)
    {
        ...
        $allProducts = collect($query->orderByDesc('default_price')->get());
        ...
        for ($i = 0; $i < 20; $i++) {
            ...
            if ($noOfProducts) {
                $candidates = $shuffled->take($noOfProducts);
                foreach ($candidates as $product) {
                    $minStep = 100 / 25;  
                    $maxStep = 10000 / 25;  
                    $randomStep = rand($minStep, $maxStep);
                    $wordCount = $product->default_wc + ($randomStep * 25);
                ...
                if ($bestMatch === null || $distance < $bestDistance) {
                    ...
                    if ($currentTotal >= $invoiceAmount * 0.90 && $currentTotal <= $invoiceAmount * 1.10) break;
                }
            } else {
                foreach ($shuffled as $product) {
                    ...
                    if ($currentTotal + $unit_price <= $maxTotal) {
                        $product->wordcount = $wordCount;
                        $product->imagecount = $imageCount;
                        $product->quantity = 1;
                        $product->unit_price = $unit_price;
